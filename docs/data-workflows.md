...
`chunk_size` is capped at 1,000. `record_limit` is exact. The existing `max_concurrency` argument is
accepted for compatibility, but record pages remain sequential to avoid unstable fixed offsets.
